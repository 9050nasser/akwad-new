...
import { createWorkDuty, deleteWorkDuty } from "@/app/actions/catalog";
...
export default async function DutiesPage({
  ...
  const rows = await prisma.workDuty.findMany({
    where: { companyId },
    orderBy: { name: "asc" },
  ...
  return (
    <PageFrame title="المهام" subtitle="مهام تشغيلية أو أنواع عمل يومي مرتبطة بالموظفين.">
      <FlashBanner notice={flash.notice} error={flash.error} />
      <form action={createWorkDuty} className="grid gap-4 md:grid-cols-2">
        <div>
              ...
              <tr key={r.id} className="hover:bg-slate-50/80">
                ...
                <td className="px-4 py-3">
                  <ConfirmServerActionForm
                    action={deleteWorkDuty}
                    confirmMessage="حذف هذه المهمة؟"
                    className="inline"
                  >
