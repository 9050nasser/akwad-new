...
export default async function DelaysReportPage({
  ...
  const [data, dropdowns, colState] = await Promise.all([
    ...
    getReportColumnState("/reports/delays", "delays", sp),
  ]);
  ...
  return (
    <PageFrame exportFileSlug="delays" title="ملخص التأخير والانصراف المبكر">
      ...
      <p className="mt-4 text-xs text-slate-500">
        شفتات متأخرة (عدد مرات): {data.delayEvents} — إجمالي التأخير:{" "}
        {formatDurationMinutes(data.rows.reduce((s, r) => s + r.delayMinutes, 0))} — إجمالي الانصراف المبكر:{" "}
        {formatDurationMinutes(totalEarlyLeave)}
      </p>

      <div className="mt-4 flex flex-wrap items-center justify-end gap-2 print:hidden">
        <ReportColumnsToolbar slug="delays" groups={DELAYS_COLUMN_GROUPS} visibleIds={visibleIds} redirectTo={returnUrl} />
      </div>
