...
export default async function EarlyInReportPage({
  ...
  const data = await getEarlyInSummary({ companyId, from, to, branchId });
  const branches = await prisma.branch.findMany({ where: { companyId }, orderBy: { name: "asc" } });

  return (
    <PageFrame
      exportFileSlug="early-in"
      title="الحضور المبكر"
      subtitle="دخول قبل بداية الشفت (لكل شفت على حدة وفق جدول الدوام الثابت)."
    >
        ...
        <table className="min-w-full text-right text-sm">
          <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
            <tr>
              ...
              <th className="px-4 py-3">شفتات مبكرة</th>
              <th className="px-4 py-3">دقائق تبكير</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100 bg-white">
            {data.rows.map((r) => (
              <tr key={`${r.name}-${r.branch}`} className="hover:bg-slate-50/80">
                ...
                <td className="px-4 py-3 text-slate-600 tabular-nums">{r.early}</td>
                <td className="px-4 py-3 text-slate-600 tabular-nums">{r.earlyMinutes}</td>
              </tr>
            ))}
          </tbody>
        </table>
