...
import { EmployeeGroupedTable } from "@/components/reports/employee-grouped-table";
...
export default async function EmployeeGroupedReportPage({
  searchParams,
}: {
  ...
  const [{ rows, capped }, dropdowns, colState] = await Promise.all([
    // نستدعي نفس دالة جلب البيانات الخاصة بالتقرير التفصيلي العادي
    getEmployeesAttendanceDetailReport({ companyId, from, to, branchId, employeeFilters }),
    loadReportFilterDropdowns(companyId),
    getReportColumnState("/reports/employee-grouped", "employees-detail", sp),
  ]);
  ...
  return (
    <PageFrame exportFileSlug="employees-grouped-detail" title="تقرير الحضور والانصراف (مجمّع للطباعة)">
      ...
      {/* استخدام المكون الجديد بدلاً من العادي */}
      <EmployeeGroupedTable rows={rows} visibleGroupIds={detailVisible} totals={totals} />
    </PageFrame>
  );
}