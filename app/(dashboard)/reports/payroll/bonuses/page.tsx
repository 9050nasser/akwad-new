...
export default async function PayrollBonusesPage({
  ...
  const [employees, bonuses, colState] = await Promise.all([
    ...
    getReportColumnState("/reports/payroll/bonuses", "payroll-bonuses", sp),
  ]);
  ...
  return (
    <PageFrame exportFileSlug="payroll-bonuses" title="إضافات يدوية (موديول الرواتب)">
      ...
      {employees.length === 0 ? (
        ...
        <form action={createPayrollManualBonus} className="mb-10 grid gap-4 rounded-xl border border-slate-200 bg-white p-5 md:grid-cols-2 print:hidden">
            ...
            {bonuses.length === 0 ? (
              ...
              bonuses.map((b) => (
                <tr key={b.id} className="hover:bg-slate-50/80">
                  {v("employee") ? <td className="px-3 py-3 font-medium text-slate-900">{b.employee.fullName}</td> : null}
                  {v("branch") ? <td className="px-3 py-3 text-slate-600">{b.employee.branch.name}</td> : null}
                  {v("month") ? <td className="px-3 py-3 tabular-nums text-slate-700">{b.bonusPeriodYm}</td> : null}
                  {v("amount") ? (
                    <td className="px-3 py-3 tabular-nums font-medium text-emerald-800">{fmtMoney(Number(b.amount))}</td>
                  ) : null}
                  {v("reason") ? <td className="max-w-xs px-3 py-3 text-slate-600">{b.reason}</td> : null}
                  {v("actions") ? (
